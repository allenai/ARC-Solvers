"""
Replicates AllenNLP's predict sub-command with special post-processing of output. Specifically,
it outputs the input json along with the entailment score prediction using a "score" field
...
class PredictCustom(Subcommand):
...
def _run(predictor: Predictor,
    ...
    def _run_predictor(batch_data):
        ...
        idx_to_label = predictor._model.vocab.get_index_to_token_vocabulary("labels")
        for model_input, output in zip(batch_data, results):
            for idx, prob in enumerate(output["label_probs"]):
                label = idx_to_label[idx]
                if label == "entails" or label == "entailment":
                    model_input["score"] = prob
